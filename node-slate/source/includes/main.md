...
店舗座席情報取得APIでは, 店舗の座席情報(座席に座れる人数, 座席の予約情報, 座席の特徴)を提供する. また, この APIがトークンの発行者によって実行結果が変化する.
...
| パラメータ | 型 | 説明 |
| --------- | --- | --- |
| restaurant_id | string | 情報を取得したい利用者アカウントのIDを表す. |
| token | string | ログイン時に発行されたtoken |


### 店舗座席情報取得APIでのレスポンスメッセージ定義(成功時)
> 店舗座席情報取得API-レスポンスメッセージの例(成功時, トークン発行者が店舗アカウント若しくは管理者の場合)
```json
{
   "jsonrpc": "2.0",
   "id": "15",
